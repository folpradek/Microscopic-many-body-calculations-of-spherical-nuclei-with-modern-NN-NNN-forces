...
function V2B_Read_OLD(Int_File::String,Params::Vector{Any},Orb::Vector{NOrb})
    HbarOmega = Params[1]
    N_max = Params[2]
    N_2max = Params[3]
    A = Params[5]
    ...
    N_2BMEs = V2B_Count(N_max,N_2max,Orb)

    v = Vector{Float64}(undef,N_2BMEs)
    read!(Int_File,v)
    N = 0
    ...
    if N != N_2BMEs
        println("\n Total number of read NN potential MEs is not equal to calculated number of MEs...\n")
    end
...
function V2B_Count_OLD(N_max::Int64,N_2max::Int64,Orb::Vector{NOrb})
    a_max = div((N_max+1)*(N_max+2),2)
    N = 0
    for a in 1:a_max
        n_a = Orb[a].n
            ...
            if (2*(n_a + n_b) + l_a + l_b) <= N_2max
                for c in 1:a
                    ...
                    for d in 1:d_max
                        ...
                        j_d = Orb[d].j
                        if ((2*(n_c + n_d) + l_c + l_d) <= N_2max) && (rem(l_a + l_b,2) == rem(l_c + l_d,2))
                            for J in div(max(abs(j_a - j_b), abs(j_c - j_d)),2):div(min(j_a + j_b, j_c + j_d),2)
                                N += 4
                            end
                        end
                    end
...
#
# WIP - Parallelized NN interaction read
#
