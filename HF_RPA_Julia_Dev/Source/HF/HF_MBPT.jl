...
function HFMBPT_Energy(Params::Vector{Any},Orb::Vector{NOrb},Orb_NN::NNOrb,N_Particle::pnInt,Particle::pnVec,N_Hole::pnInt,Hole::pnVec,VNN_res::NNInt)
...
function HFMBPT_Density(Params::Vector{Any},Orb::Vector{NOrb},Orb_NN::NNOrb,N_Particle::pnInt,Particle::pnVec,N_Hole::pnInt,Hole::pnVec,VNN_res::NNInt)
    N_max = Params[7]
            ...
            if j_p == j_h && l_p == l_h
                ...
                @inbounds for f in 1:N_Hole.p
                    ...
                    @inbounds for J in div(abs(j_p - j_f),2):div((j_p + j_f),2)
                        Hat = Float64(2*J + 1) / Float64(j_p + 1)
                        @inbounds for r in 1:N_Particle.p
                            ...
                            @inbounds for q in 1:N_Particle.p
                                ...
                                if (abs(j_q - j_r) <= 2*J) && (2*J <= (j_q + j_r)) && (rem(l_q + l_r, 2) == rem(l_h + l_f, 2)) && 
                                    ...
                                    ME = Hat * V2B(a_p,a_f,a_q,a_r,J,1,VNN_res.pp,Orb,Orb_NN) * 
                        ...
                        @inbounds for f in 1:N_Hole.p
                            ...
                            @inbounds for g in 1:N_Hole.p
                                ...
                                if (abs(j_g - j_f) <= 2*J) && (2*J <= (j_g + j_f)) && (rem(l_p + l_r, 2) == rem(l_g + l_f, 2)) && 
                                    ...
                                    ME = Float64(2*J + 1) / Float64(j_p + 1) * V2B(a_p,a_r,a_g,a_f,J,1,VNN_res.pp,Orb,Orb_NN) * 
                    ...
                    @inbounds for q in 1:N_Particle.p
                        ...
                        if (abs(j_p - j_q) <= 2*J) && (2*J <= (j_p + j_q)) && (rem(l_h + l_g, 2) == rem(l_p + l_q, 2))
                            ...
                            @inbounds for c in 1:N_Hole.p
                                ...
                                E_c = Hole.p[c].E
                                chSum = 0.0
                                if (rem(l_g + l_c, 2) == rem(l_q + l_p, 2)) && (rem(l_p + l_q + l_h + l_g, 2) == rem(l_h + l_c, 2)) && (j_c == j_h) && (l_c == l_h)
                        ...
                        @inbounds for r in 1:N_Particle.n
                            ...
                            @inbounds for q in 1:N_Particle.n
                                ...
                                if (abs(j_q - j_r) <= 2*J) && (2*J <= (j_q + j_r)) && (rem(l_q + l_r, 2) == rem(l_h + l_f, 2)) && 
                                    ...
                                    ME = Float64(2*J + 1) / Float64(j_p + 1) * V2B(a_p,a_f,a_q,a_r,J,1,VNN_res.nn,Orb,Orb_NN) * 
                        ...
                        @inbounds for f in 1:N_Hole.p
                            ...
                            @inbounds for g in 1:N_Hole.p
                                ...
                                if (abs(j_g - j_f) <= 2*J) && (2*J <= (j_g + j_f)) && (rem(l_p + l_r, 2) == rem(l_g + l_f, 2)) && 
                                    ...
                                    ME = Float64(2*J + 1) / Float64(j_p + 1) * V2B(a_p,a_r,a_g,a_f,J,1,VNN_res.nn,Orb,Orb_NN) * 
