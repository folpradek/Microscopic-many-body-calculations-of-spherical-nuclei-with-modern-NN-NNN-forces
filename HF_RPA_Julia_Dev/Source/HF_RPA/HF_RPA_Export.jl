...
function HF_RPA_Summary(Params::Vector{Any},N_nu::Matrix{Int64},E_Corr_RPA::Float64)
    ...
    if Orthogon == true
        Output_Path = Output_File * "/HF_RPA_Summary_Ortho.dat"
    else
        Output_Path = Output_File * "/HF_RPA_Summary_Spur.dat"
    end
...
function HF_RPA_Spectrum_Export(Params::Vector{Any},N_nu::Matrix{Int64},E_TDA::Matrix{Vector{Float64}},E_RPA::Matrix{Vector{ComplexF64}},s_TDA::Matrix{Vector{Float64}},s_RPA::Matrix{Vector{Float64}})
    ...
    if Orthogon == true
        Output_Path_TDA = Output_File * "/Spectra/TDA_Ortho.dat"
        Output_Path_RPA = Output_File * "/Spectra/RPA_Ortho.dat"
    else
        Output_Path_TDA = Output_File * "/Spectra/TDA_Spur.dat"
        Output_Path_RPA = Output_File * "/Spectra/RPA_Spur.dat"
    end
...
function HF_RPA_Plot_Spectrum_Export(Params::Vector{Any},N_nu::Matrix{Int64},E_TDA::Matrix{Vector{Float64}},E_RPA::Matrix{Vector{ComplexF64}})
    ...
    if Orthogon == true
        Output_Path_TDA = Output_File * "/Spectra/TDA_Plot_Ortho.dat"
        Output_Path_RPA = Output_File * "/Spectra/RPA_Plot_Ortho.dat"
    else
        Output_Path_TDA = Output_File * "/Spectra/TDA_Plot_Spur.dat"
        Output_Path_RPA = Output_File * "/Spectra/RPA_Plot_Spur.dat"
    end
...
function HF_RPA_Transition_Export(Params::Vector{Any},N_nu::Matrix{Int64},E_TDA::Matrix{Vector{Float64}},E_RPA::Matrix{Vector{ComplexF64}},rB_TDA::ReducedTransition,rB_RPA::ReducedTransition)
    ...
    # TDA E0 export
    open(Output_File * "/Transitions/E0/TDA_E0.dat", "w") do Write_File
        J = 0
        P = 1
        N_ph = N_nu[J+1,P]
    ...
    # RPA E0 export
    open(Output_File * "/Transitions/E0/RPA_E0.dat", "w") do Write_File
        J = 0
        P = 1
        N_ph = N_nu[J+1,P]
        println(Write_File, "E\t\tB_ph")
    ...
    if Orthogon == true
        # TDA E1 export
        open(Output_File * "/Transitions/E1/TDA_E1_Ortho.dat", "w") do Write_File
            J = 1
            P = 2
            N_ph = N_nu[J+1,P]
            println(Write_File, "E\t\tB_ph")
        ...
        # RPA E1 export
        open(Output_File * "/Transitions/E1/RPA_E1_Ortho.dat", "w") do Write_File
            J = 1
            P = 2
            N_ph = N_nu[J+1,P]
            println(Write_File, "E\t\tB_ph")
        ...
        # TDA E1 export
        open(Output_File * "/Transitions/E1/TDA_E1_Spur.dat", "w") do Write_File
            J = 1
            P = 2
            N_ph = N_nu[J+1,P]
            println(Write_File, "E\t\tB_ph")
        ...
        # RPA E1 export
        open(Output_File * "/Transitions/E1/RPA_E1_Spur.dat", "w") do Write_File
            J = 1
            P = 2
            N_ph = N_nu[J+1,P]
            println(Write_File, "E\t\tB_ph")
    ...
    # TDA E2 export
    open(Output_File * "/Transitions/E2/TDA_E2.dat", "w") do Write_File
        J = 2
        P = 1
        N_ph = N_nu[J+1,P]
        println(Write_File, "E\t\tB_ph")
    ...
    # RPA E2 export
    open(Output_File * "/Transitions/E2/RPA_E2.dat", "w") do Write_File
        J = 2
        P = 1
        N_ph = N_nu[J+1,P]
        println(Write_File, "E\t\tB_ph")
    ...
    # TDA E3 export
    open(Output_File * "/Transitions/E3/TDA_E3.dat", "w") do Write_File
        J = 3
        P = 2
        N_ph = N_nu[J+1,P]
        println(Write_File, "E\t\tB_ph")
    ...
    # RPA E3 export
    open(Output_File * "/Transitions/E3/RPA_E3.dat", "w") do Write_File
        J = 3
        P = 2
        N_ph = N_nu[J+1,P]
        println(Write_File, "E\t\tB_ph")
...
function HF_RPA_S0_Export(Params::Vector{Any},N_nu::Matrix{Int64},E_TDA::Matrix{Vector{Float64}},E_RPA::Matrix{Vector{ComplexF64}},rB_TDA::ReducedTransition,rB_RPA::ReducedTransition)
    ...
    E0_TDA_grid = zeros(Float64,N_grid)
    E1_TDA_grid = zeros(Float64,N_grid)
    E2_TDA_grid = zeros(Float64,N_grid)
    E3_TDA_grid = zeros(Float64,N_grid)
    E0is_TDA_grid = zeros(Float64,N_grid)
    E1is_TDA_grid = zeros(Float64,N_grid)
    E2is_TDA_grid = zeros(Float64,N_grid)
    E3is_TDA_grid = zeros(Float64,N_grid)
    E0iv_TDA_grid = zeros(Float64,N_grid)
    E1iv_TDA_grid = zeros(Float64,N_grid)
    E2iv_TDA_grid = zeros(Float64,N_grid)
    E3iv_TDA_grid = zeros(Float64,N_grid)
    ...
    open(Output_File * "/Transitions/E0/TDA_E0_S0.dat", "w") do Write_File
        @inbounds for (i, E) in enumerate(E_grid)
    ...
    open(Output_File * "/Transitions/E0/RPA_E0_S0.dat", "w") do Write_File
        @inbounds for (i, E) in enumerate(E_grid)
    ...
    if Orthogon == true
        open(Output_File * "/Transitions/E1/TDA_E1_S0_Ortho.dat", "w") do Write_File
            @inbounds for (i, E) in enumerate(E_grid)
        ...
        open(Output_File * "/Transitions/E1/RPA_E1_S0_Ortho.dat", "w") do Write_File
            @inbounds for (i, E) in enumerate(E_grid)
        ...
        open(Output_File * "/Transitions/E1/TDA_E1_S0_Spur.dat", "w") do Write_File
            @inbounds for (i, E) in enumerate(E_grid)
                Row = string(round(E,digits=6)) * "\t" * string(round(E1_TDA_grid[i],digits=6)) * "\t" *
                        string(round(E1is_TDA_grid[i],digits=6)) * "\t" * string(round(E1iv_TDA_grid[i],digits=6))
                println(Write_File, Row)
        ...
        open(Output_File * "/Transitions/E1/RPA_E1_S0_Spur.dat", "w") do Write_File
            @inbounds for (i, E) in enumerate(E_grid)
                Row = string(round(E,digits=6)) * "\t" * string(round(E1_RPA_grid[i],digits=6)) * "\t" *
                        string(round(E1is_RPA_grid[i],digits=6)) * "\t" * string(round(E1iv_RPA_grid[i],digits=6))
                println(Write_File, Row)
    ...
    open(Output_File * "/Transitions/E2/TDA_E2_S0.dat", "w") do Write_File
        @inbounds for (i, E) in enumerate(E_grid)
    ...
    open(Output_File * "/Transitions/E2/RPA_E2_S0.dat", "w") do Write_File
        @inbounds for (i, E) in enumerate(E_grid)
    ...
    open(Output_File * "/Transitions/E3/TDA_E3_S0.dat", "w") do Write_File
        @inbounds for (i, E) in enumerate(E_grid)
    ...
    open(Output_File * "/Transitions/E3/RPA_E3_S0.dat", "w") do Write_File
        @inbounds for (i, E) in enumerate(E_grid)
...
function HF_RPA_Sigma_Export(Params::Vector{Any},N_nu::Matrix{Int64},E_TDA::Matrix{Vector{Float64}},E_RPA::Matrix{Vector{ComplexF64}},rB_TDA::ReducedTransition,rB_RPA::ReducedTransition)
    ...
    N_grid = 25000
    E_grid = range(E_min, stop = E_max, length = N_grid)
    SigmaE1TDA_grid = zeros(Float64,N_grid)
    SigmaE1RPA_grid = zeros(Float64,N_grid)
    SigmaE2TDA_grid = zeros(Float64,N_grid)
    SigmaE2RPA_grid = zeros(Float64,N_grid)
    SigmaE3TDA_grid = zeros(Float64,N_grid)
    ...
    if Orthogon == true
        open(Output_File * "/Transitions/E1/TDA_Sigma_E1_Ortho.dat", "w") do Write_File
            @inbounds for (i, E) in enumerate(E_grid)
                Row = string(round(E,digits=6)) * "\t" * string(round(SigmaE1TDA_grid[i],digits=6))
                println(Write_File, Row)
            end
        end

        open(Output_File * "/Transitions/E1/RPA_Sigma_E1_Ortho.dat", "w") do Write_File
            @inbounds for (i, E) in enumerate(E_grid)
        ...
        open(Output_File * "/Transitions/E1/TDA_Sigma_E1_Spur.dat", "w") do Write_File
            @inbounds for (i, E) in enumerate(E_grid)
                Row = string(round(E,digits=6)) * "\t" * string(round(SigmaE1TDA_grid[i],digits=6))
                println(Write_File, Row)
            end
        ...
        open(Output_File * "/Transitions/E1/RPA_Sigma_E1_Spur.dat", "w") do Write_File
            @inbounds for (i, E) in enumerate(E_grid)
                Row = string(round(E,digits=6)) * "\t" * string(round(SigmaE1RPA_grid[i],digits=6))
                println(Write_File, Row)
            end
    ...
    open(Output_File * "/Transitions/E2/TDA_Sigma_E2.dat", "w") do Write_File
        @inbounds for (i, E) in enumerate(E_grid)
            Row = string(round(E,digits=6)) * "\t" * string(round(SigmaE2TDA_grid[i],digits=6))
            println(Write_File, Row)
        end
    end

    open(Output_File * "/Transitions/E2/RPA_Sigma_E2.dat", "w") do Write_File
        @inbounds for (i, E) in enumerate(E_grid)
    ...
    open(Output_File * "/Transitions/E3/TDA_Sigma_E3.dat", "w") do Write_File
        @inbounds for (i, E) in enumerate(E_grid)
            Row = string(round(E,digits=6)) * "\t" * string(round(SigmaE3TDA_grid[i],digits=6))
            println(Write_File, Row)
        end
    end

    open(Output_File * "/Transitions/E3/RPA_Sigma_E3.dat", "w") do Write_File
        @inbounds for (i, E) in enumerate(E_grid)
