...
function V3B_NO2B_Read(Int_File::String,Params::Vector{Any},Orb::Vector{NOrb})
    ...
    #N_elements = V3B_NO2B_Count(N_max,N_2max,N_3max,Orb)
    ...
    open(Int_File, "r") do Bin_Read
        ...
        #if N_total != N_3BMEs
        #    println("\n Total number of read MEs is not equal to calculates number of MEs...\n")
        #end
...
function V3B_NO2B_Count(N_max::Int64,N_2max::Int64,N_3max::Int64,Orb::Vector{NOrb})
    a_max = div((N_max+1)*(N_max+2),2)
    N = 0
    N_partial = Threads.Atomic{Int64}[Threads.Atomic{Int64}(0) for _ in 1:Threads.nthreads()]
