...
function Make_ParticleHole_Orbitals(N_max::Int64,Input_File::String,Orb::Vector{NOrb})
    ...
    N_particle = pnInt(N_pParticle,N_nParticle)
    N_hole = pnInt(N_pHole,N_nHole)

    Particle = pnVec(pParticle_vec,nParticle_vec)
    Hole = pnVec(pHole_vec,nHole_vec)
...
function Make_Phonon_Orbitals(N_Particle::pnInt,Particle::pnVec,N_Hole::pnInt,Hole::pnVec)
    N_Phonon = 0
...
function Phonon_Ind(ph::Int64,Phonon::Vector{PhState},Particle::pnVec,Hole::pnVec)
    p = Phonon[ph].p
    h = Phonon[ph].h
    t_ph = Phonon[ph].tz
    J = Phonon[ph].J
...
function Orbitals_Export(Params::Vector{Any},Orb::Vector{NOrb})
    ...
    pE_Import = "IO/" * Output_File * "/pE.bin"
    pSPE = Vector{Float64}(undef,a_max)
    open(pE_Import, "r") do Read_File
        @inbounds for a in 1:a_max
            ME = read(Read_File, Float64)
    ...
    nE_Import = "IO/" * Output_File * "/nE.bin"
    nSPE = Vector{Float64}(undef,a_max)
    open(nE_Import, "r") do Read_File
        @inbounds for a in 1:a_max
            ME = read(Read_File, Float64)
