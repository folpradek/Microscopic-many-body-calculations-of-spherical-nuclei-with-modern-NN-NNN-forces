function HF_ERPA_0_Allocate(Params::Vector{Any},N_nu::Matrix{Int64},Orb_Phonon::Matrix{Vector{Int64}},Phonon::Vector{PhState},Particle::pnVec,Hole::pnVec,Orb::Vector{NOrb},Orb_NN::NNOrb,VNN::NNInt)
                ...
                if rem(l_p + l_q, 2) == rem(l_h + l_g, 2)
                    @inbounds for J_r in div(abs(j_p - j_q),2):div((j_p + j_q),2)
                        if (abs(j_h - j_g) <= 2*J_r) && (2*J_r <= (j_h + j_g))
                            ...
                            if t_ph == -1 && t_qg == -1
                                Phase = 1/sqrt(Float64(1 + 0*KroneckerDelta(a_p,a_q))*Float64(1 + 0*KroneckerDelta(a_h,a_g))) * Float64((-1)^(div(j_h + j_q,2) + J + J_r))
...
function HF_ERPA_Allocate(Params::Vector{Any},N_nu::Matrix{Int64},Orb_Phonon::Matrix{Vector{Int64}},Phonon::Vector{PhState},Particle::pnVec,Hole::pnVec,Orb::Vector{NOrb},Orb_NN::NNOrb,VNN::NNInt,pRho::Matrix{Float64},nRho::Matrix{Float64})
    # Read calculation parameters ...
    N_max = Params[4]
    N_2max = 2*N_max
    J_max = N_2max + 1
