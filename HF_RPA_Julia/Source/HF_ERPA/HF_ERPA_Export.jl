function HF_ERPA_Export(Params::Vector{Any},Orb::Vector{NOrb},N_nu::Matrix{Int64},E_0_RPA::Float64,E_RPA::Matrix{Vector{ComplexF64}},s_RPA::Matrix{Vector{Float64}},rB_RPA::ReducedTransition,pRho::Matrix{Float64},nRho::Matrix{Float64})
...
function HF_ERPA_Summary(Params::Vector{Any},N_nu::Matrix{Int64},E_0::Float64)
    ...
    if Orthogon == true
        Output_Path = Output_File * "/HF_ERPA_Summary_Ortho.dat"
    else
        Output_Path = Output_File * "/HF_ERPA_Summary_Spur.dat"
    end
    ...
    Summary_File =  open(Output_Path, "w")
        ...
        println(Summary_File, "\n\tTo get the total correct ground-state energy add the HF-mean field energy & subtract total hole states energy ...")
    close(Summary_File)
...
function HF_ERPA_Occupation_Export(Params::Vector{Any},Orb::Vector{NOrb},pRho::Matrix{Float64},nRho::Matrix{Float64})
    ...
    if Orthogon == true
        Output_Path_RPA = Output_File * "/Densities/ERPA_Occupation_Ortho.dat"
    else
        Output_Path_RPA = Output_File * "/Densities/ERPA_Occupation_Spur.dat"
    end
...
function HF_ERPA_Transition_Export(Params::Vector{Any},N_nu::Matrix{Int64},E_RPA::Matrix{Vector{ComplexF64}},rB_RPA::ReducedTransition)
    ...
    # ERPA E0 export
    open(Output_File * "/Transitions/E0/ERPA_E0.dat", "w") do Write_File
        J = 0
        P = 1
        N_ph = N_nu[J+1,P]
    ...
    if Orthogon == true
        ...
        # ERPA E1 export
        open(Output_File * "/Transitions/E1/ERPA_E1_Ortho.dat", "w") do Write_File
            J = 1
            P = 2
            N_ph = N_nu[J+1,P]
            println(Write_File, "E\t\tB_ph")
        ...
        # ERPA E1 export
        open(Output_File * "/Transitions/E1/ERPA_E1_Spur.dat", "w") do Write_File
            J = 1
            P = 2
            N_ph = N_nu[J+1,P]
            println(Write_File, "E\t\tB_ph")
    ...
    # ERPA E2 export
    open(Output_File * "/Transitions/E2/ERPA_E2.dat", "w") do Write_File
        J = 2
        P = 1
        N_ph = N_nu[J+1,P]
        println(Write_File, "E\t\tB_ph")
    ...
    # ERPA E3 export
    open(Output_File * "/Transitions/E3/ERPA_E3.dat", "w") do Write_File
        J = 3
        P = 2
        N_ph = N_nu[J+1,P]
        println(Write_File, "E\t\tB_ph")
...
function HF_ERPA_S0_Export(Params::Vector{Any},N_nu::Matrix{Int64},E_RPA::Matrix{Vector{ComplexF64}},rB_RPA::ReducedTransition)
    ...
    open(Output_File * "/Transitions/E0/ERPA_E0_S0.dat", "w") do Write_File
        @inbounds for (i, E) in enumerate(E_grid)
    ...
    if Orthogon == true
        open(Output_File * "/Transitions/E1/ERPA_E1_S0_Ortho.dat", "w") do Write_File
            @inbounds for (i, E) in enumerate(E_grid)
        ...
        open(Output_File * "/Transitions/E1/ERPA_E1_S0_Spur.dat", "w") do Write_File
            @inbounds for (i, E) in enumerate(E_grid)
                Row = string(round(E,digits=6)) * "\t" * string(round(E1_RPA_grid[i],digits=6)) * "\t" *
                        string(round(E1is_RPA_grid[i],digits=6)) * "\t" * string(round(E1iv_RPA_grid[i],digits=6))
                println(Write_File, Row)
    ...
    open(Output_File * "/Transitions/E2/ERPA_E2_S0.dat", "w") do Write_File
        @inbounds for (i, E) in enumerate(E_grid)
    ...
    open(Output_File * "/Transitions/E3/ERPA_E3_S0.dat", "w") do Write_File
        @inbounds for (i, E) in enumerate(E_grid)
...
function HF_ERPA_Sigma_Export(Params::Vector{Any},N_nu::Matrix{Int64},E_RPA::Matrix{Vector{ComplexF64}},rB_RPA::ReducedTransition)
    ...
    if Orthogon == true
        open(Output_File * "/Transitions/E1/ERPA_Sigma_E1_Ortho.dat", "w") do Write_File
            @inbounds for (i, E) in enumerate(E_grid)
        ...
        open(Output_File * "/Transitions/E1/ERPA_Sigma_E1_Spur.dat", "w") do Write_File
            @inbounds for (i, E) in enumerate(E_grid)
                Row = string(round(E,digits=6)) * "\t" * string(round(SigmaE1RPA_grid[i],digits=6))
                println(Write_File, Row)
            end
    ...
    open(Output_File * "/Transitions/E2/ERPA_Sigma_E2.dat", "w") do Write_File
        @inbounds for (i, E) in enumerate(E_grid)
    ...
    open(Output_File * "/Transitions/E3/ERPA_Sigma_E3.dat", "w") do Write_File
        @inbounds for (i, E) in enumerate(E_grid)
